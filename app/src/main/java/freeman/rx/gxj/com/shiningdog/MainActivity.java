...
import freeman.rx.gxj.com.shiningdog.view.CircularImageView;
...
public class MainActivity extends Activity {
    ...
    private void initList()    {
        // Add header
        View headerView = ((LayoutInflater) getSystemService(
                Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.header, mList, false);
        mList.addHeaderView(headerView, null, false);

        // Add footer
        View footerView = ((LayoutInflater) getSystemService(
                Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.footer, mList, false);
        mList.addFooterView(footerView, null, false);

        // DOGE!!!
        final CircularImageView imgDoge = (CircularImageView)
                headerView.findViewById(R.id.imgDoge);
        ...
        Picasso.with(this).load(URL).placeholder(R.mipmap.default_avatar).
                error(R.mipmap.grumpy_cat).transform(new RoundTransform()).into(imgNetwork);
